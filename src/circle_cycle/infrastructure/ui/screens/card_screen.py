"""Card selection screen for applying buffs between rounds."""
...
class CardScreen(tk.Frame):
    ...
    def refresh(self) -> None:
        ...
        for index, card in enumerate(cards):
            ...
            panel.bind("<Button-1>", self._make_card_handler(card))
        ...
        for index, character in enumerate(self.app.engine.player_team):
            button = tk.Button(
                ...
                fg="white",
                font=("Arial", 12, "bold"),
                command=self._make_target_command(character),
            )
    ...
    def select_target(self, target: Character) -> None:
    ...
    def _make_card_handler(self, card: Card) -> Callable[[tk.Event[tk.Frame]], None]:
        """Create an event handler for selecting a specific card."""

        def _handler(event: tk.Event[tk.Frame]) -> None:
            self.select_card(card)

        return _handler

    def _make_target_command(self, character: Character) -> Callable[[], None]:
        """Create a callback for selecting a specific target character."""

        def _command() -> None:
            self.select_target(character)
